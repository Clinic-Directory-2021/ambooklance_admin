  ...
  const startBooking = async(data) =>{
    ...
    .then(async()=>{
      ...
      setDoc(doc(Notifications, booking_id), {
        message:'Your request has been approved and your driver is Mr/Ms '+data['label']+'. Check your map to track your driver.',
        date: (current_date.getMonth() + 1) + " / " + (current_date.getDay() + 1) + " / " + current_date.getFullYear(),
       });
       setDoc(doc(DriverNotif, booking_id), {
        message:'Hello Mr/Ms ' + data['label']+ ' You have been assign to drive ambulance for a resident. Please kindly check your map tab',
        date: (current_date.getMonth() + 1) + " / " + (current_date.getDay() + 1) + " / " + current_date.getFullYear(),
       });
    ...
    <View style={styles.container}>
     ...
     emergencyBookings.map((data)=>{
       if(data['status'] == 'on the way'){
        ...
        return(
          <Card style={{padding: 10, margin: 10, flexDirection:'row'}}>
           ...
           <View style={{width:'30%'}}>
                 <TouchableOpacity style={{marginBottom:'auto', marginStart:'auto', marginEnd:'auto',  padding:5, marginTop:'auto'}} onPress={()=>{setDenyModalVisible(!denyModalVisible)}}>
       ...
       <Card style={{padding: 10, margin: 10, flexDirection:'row'}}>
        ...
        <View style={{width:'30%'}}>
              <TouchableOpacity style={{marginTop:'auto', marginStart:'auto', marginEnd:'auto', padding:5}} onPress={()=>{
                setAcceptModalVisible(!acceptModalVisible)
              ...
              <TouchableOpacity style={{marginBottom:'auto', marginStart:'auto', marginEnd:'auto',  padding:5}} onPress={()=>{setDenyModalVisible(!denyModalVisible)}}>
